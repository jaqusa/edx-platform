...
class Command(BaseCommand):
    ...
    def unenroll_users_from_csv(self, csv_file):
        ...
        is_unenroll_all = len(reader[0].keys()) == 1 and 'course_id' in reader[0].keys()

        if is_unenroll_all:
            for row in reader:
                self.unenroll_learners(row['course_id'])
        else:
            for row in reader:
                self.unenroll_learners(row['course_id'], username=row['username'])
    ...
    def unenroll_learners(self, course_id, username=None):
        ...
        for enrollment in enrollments:
            ...
            logger.info("User [{}] have been successfully unenrolled from the course: {}"
                        .format(enrollment.user.username, course_id))
